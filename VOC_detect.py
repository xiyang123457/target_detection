...
for xml_path in glob.glob(os.path.join(ANN, "*.xml")):
    ...
    for obj in objs:
        ...
        x2 = float(bb.find("xmax").text)    # 变量 y1：框左上角 y 坐标
...
plt.savefig("outputs/fig1_class.png", dpi=140)
...
plt.ylabel("images")
plt.tight_layout()
plt.savefig("outputs/fig2_per_image.png", dpi=140)
plt.close()

# 图3：框宽高 —— 看尺度
plt.figure(figsize=(8, 4))
plt.hist(w_list, bins=50, alpha=0.6, label="width")    # alpha 半透明，重叠处才看得见
plt.hist(h_list, bins=50, alpha=0.6, label="height")
plt.legend()
plt.xlabel("pixels")
plt.tight_layout()
plt.savefig("outputs/fig3_size.png", dpi=140)
plt.close()